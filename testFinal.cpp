...
//Test that check for bigger input
...
bool advancedTest4(void* DS){
    int min=0;

    ASSERT(addGladiator(DS, 10, 10, 1) == SUCCESS);
    ASSERT(addGladiator(DS, 20, 20, 2) == SUCCESS);
    ASSERT(trainingGroupFight(DS,1,2,1,1)==SUCCESS);
    ASSERT(getMinTrainingGroup(DS,&min)==SUCCESS);
    ASSERT(min==2);


    for(int i=1;i<=100;i++) {
        ASSERT(addGladiator(DS, 200+i, i, 2) == SUCCESS);
    }
    for(int i=1;i<=101;i++) {
        ASSERT(addGladiator(DS, 300+i, i, 3) == SUCCESS);
    }

    ASSERT(trainingGroupFight(DS,2,3,100,100)==SUCCESS);
    ASSERT(getMinTrainingGroup(DS,&min)==SUCCESS);
    ASSERT(min==3);
